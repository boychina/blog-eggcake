...
本篇是 **《AI 技术演进与核心算法实战：从原理到 Agent》全景系列** 的第一篇，隶属于 **【第一模块：基石篇 —— 数学直觉与模型原理】**。我们将一起溯源 AI 的发展简史，理清技术演进脉络，理解 AI 到底是如何从刻板的规则走向智能的“思考”的。
...
- **组合爆炸**：如果一个系统只有 10 个布尔变量，那就有 $2^{10}=1024$ 种组合；如果是 100 个变量呢？人类根本无法穷举和手动编写所有的 `If-Then` 规则。
...
1. **遗忘问题（长距离依赖瓶颈）**：RNN 在处理序列时，每一步的隐藏状态都要包含之前所有的信息。但无论句子多长，RNN 都必须把它压缩成一个**固定长度的向量**。这就好比用一个固定大小的杯子去装无限的水，当序列过长时，早期的信息会被后来的信息不断稀释甚至覆盖（数学上常表现为梯度消失），这就导致了模型“读到句尾，忘了句首”，极难准确捕捉长距离的上下文关联。
2. **无法并行计算**：RNN 的时序结构决定了第 `t` 步的计算必须依赖第 `t-1` 步的输出结果。这意味着机器只能像人类阅读一样，按顺序一个词一个词地串行处理，无法同时计算整句话。在现代 GPU 拥有成千上万个核心的背景下，这种串行机制造成了算力的巨大浪费，导致模型训练效率极低，根本无法在海量数据集上进行大规模扩展（Scaling）。
...
这种机制极其优雅地解决了 RNN 遗留的两大死穴：

1. **破除长距离依赖（解决遗忘问题）**：在自注意力机制中，任意两个词之间的计算距离永远是 **1**。不管两个词在句子里隔了十个字还是一万个字，它们都可以通过 Q 和 K 的直接点积瞬间建立联系。模型不再需要像 RNN 那样把信息在隐状态里苦苦传递，信息损耗降到了零。
2. **实现完全并行化（解决串行瓶颈）**：自注意力的计算完全基于矩阵乘法（Matrix Multiplication）。当前词的计算不再依赖上一个词的计算结果。这意味着一整段话输入进去，所有词的 Q、K、V 匹配可以在 GPU 中**同时并行计算**。

正是由于打破了计算瓶颈，Transformer 使得模型可以无限制地吸收算力和数据进行堆叠参数，从而迎来了大模型时代的“Scaling Law（缩放定律）”，为后来 ChatGPT 的涌现奠定了最核心的算力基础。
